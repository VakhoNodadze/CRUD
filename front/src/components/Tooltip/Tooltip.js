...
const Tooltip = ({
  ...
  useEffect(
    () => {
      if (tooltipRef.current) setDimensionsTip({ height: tooltipRef.current.clientHeight, width: tooltipRef.current.clientWidth })
    },
    [tooltipRef.current]
  )

  return (
    <StyledContainer ref={containerRef} onMouseEnter={handleShowTip} onMouseLeave={handleHideTip} {...rest}>
      ...
      {visible && (
        <Portal>
          ...
          >
            <StyledTipContent className="content" position={currentPosition} dimensionsTip={dimensionsTip}>
